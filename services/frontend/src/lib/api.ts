...
export type { UploadProgressUpdate, UploadInitiateResponse, ChunkUploadResponse, UploadCompleteResponse }
...
export async function getProcessingProgress(videoId: string): Promise<import('@/types').ProgressResponse> {
  const response = await fetch(`${API_BASE_URL}/videos/${videoId}/progress`)

  if (!response.ok) {
    throw new ApiError(response.status, 'Failed to get processing progress')
  }

  return response.json()
}

export async function getDownloadUrl(videoId: string): Promise<{ download_url: string; expires_in: number }> {
  const response = await fetch(`${API_BASE_URL}/videos/${videoId}/download`)

  if (!response.ok) {
    if (response.status === 404) {
      throw new ApiError(404, 'Processed video not yet available')
    }
